...
- `proposal-media-investment.html` — private client proposal page for a media-investment/maternity-leave-replacement mandate (Plum Living-style brief, >600k€ media budget). Noindex, unlinked, not in sitemap. **French only, no EN twin yet.** Uses the same `why-section` (3 convictions) and `exp-grid` (3 dark cards, "Ce que nous ferions") patterns. Section order here is intentionally different from the Wanteeed page: hero → intro → "Qui sommes nous" (02) → convictions (03) → approche (04) — the "who we are" block comes right after the intro, not at the end. "Qui sommes nous" duplicates the real homepage `.hero` section structure (`.hero-dup`/`.hero-grid`/`.hcard`, real 6 profile photos, real tags) instead of a beige card. A `.stat-list` (bulleted stats: +250 profils, +100 missions, +15 rôles, +7 ans) sits under that section's subtext — this is a deliberate exception to the site's "no bullet lists" rule, done on explicit request for this one page only, not a precedent for the public site. The founder signature (photo/name/title/Calendly button) was moved out of any section into its own standalone `.closing-signature` strip right above the footer — small (56px photo), single row, not boxed. Assets: `hero-video-media-investment.mp4` (renamed to avoid colliding with the Wanteeed page's `hero-video.mp4`) and `Assets/Images/Profiles/audra-rosales.jpg` (the founder photo, extracted from the local-mockup base64 embed into a real file before shipping, per rule 7 below).
...
| `proposal-media-investment.html` (FR only) | Unnamed company so far — maternity-leave replacement mandate, >600k€/an media investment budget (brief reads like Plum Living, a French/German interior-design brand, but this was never explicitly confirmed by the user — verify before stating the company name to anyone) | Live on `main` | `dev-metodas.a-rosales.workers.dev/proposal-media-investment` | `metodas.co/proposal-media-investment` |
