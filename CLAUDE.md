...
**STANDING RULE — 2-week auto-expiry:** every proposal landing page gets taken off `main` (production) after 2 weeks of no news from the prospect it was built for, counted from the "Shipped" date in the registry below, unless the user has explicitly said to keep it live longer. At the session-start check-in above, if a page's Shipped date is 2+ weeks in the past, flag it specifically — "it's been X weeks since [page] went live, still no news you mentioned — take it down?" — rather than only offering a generic deletion prompt. Taking down means removing it from `main` (and its unique assets); it's fine to leave it on `dev` a little longer if the user wants a final look before full deletion. Update the registry's Shipped date whenever a page is first pushed to `main`, and update/remove the row once it's taken down.
