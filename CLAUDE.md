...
| `proposal-documan.html` (FR only) | **Documan** — Senior Growth Lead mandate, augmented by AI/automation infra built by Documan's own team. 3 convictions, a new "Notre vision" section (duplicated `why-section`, sits between Conviction and Profils), "Pourquoi nous choisir" replaced entirely by the real homepage Tarifs/pricing-box section (25% figure kept, new 5-item checkmark list). Profiles: Romain Lacoste, Sébastien Chaix, Baptiste Moisan — each has a short "Rôle potentiel" label (Lead Growth / Lead Growth / Expertise ponctuelle) instead of the longer mission-sentence style used on the media-investment page. No hero image/video (none provided). | Live on `dev` only — not yet shipped to `main` | 2026-08-05 (dev only) | `dev-metodas.a-rosales.workers.dev/proposal-documan` | not yet shipped |
