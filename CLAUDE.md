...
| `proposal-media-investment.html` (FR only) | Unnamed company so far — maternity-leave replacement mandate, >600k€/an media investment budget (brief reads like Plum Living, a French/German interior-design brand, but this was never explicitly confirmed by the user — verify before stating the company name to anyone). Includes a "Profils" section featuring Romain Lacoste, Katia Borodina, Baptiste Moisan. | Live on `main`, prod and dev in sync as of 2026-08-04 | `dev-metodas.a-rosales.workers.dev/proposal-media-investment` | `metodas.co/proposal-media-investment` |
