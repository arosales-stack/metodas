...
| `proposal-verisure.html` (FR only) | **Verisure** — UX/CRO freelance mandate (alarm/security company, ~6M clients, ~5M leads/year). Single featured profile: Thomas (Senior Product & UX Designer). 4 convictions, 4-step "Comment nous procéderions" (not 3, per this specific brief). | Live on `dev` only — not yet shipped to `main` | 2026-08-05 (dev only) | `dev-metodas.a-rosales.workers.dev/proposal-verisure` | not yet shipped |
